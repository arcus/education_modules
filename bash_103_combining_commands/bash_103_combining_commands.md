...
In the Bash language, right arrows `>` redirect output, and left arrows `<` redirect input. We won't cover redirecting output in this module, however, if you end up writing more complicated pipelines in the future it may be useful to [learn more about redirecting output](https://www.gnu.org/software/bash/manual/html_node/Redirections.html).
